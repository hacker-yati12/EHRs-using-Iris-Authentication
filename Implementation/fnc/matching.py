...
def matching(template_extr, mask_extr, temp_dir, threshold=0.38):
	...
	result_list=[]
	for f in sorted(listdir(temp_dir)):
		r_list = matchingPool(f,template_extr,mask_extr,temp_dir)
	...
	ind_thres=0
	for i in range(len(hm_dists)):
		if(hm_dists[i]<=threshold):
			ind_thres=i
	...
	filenames = filenames[ind_thres]
	return (filenames,hm_dists)
